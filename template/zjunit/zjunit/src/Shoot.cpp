...
PlayerTask Shoot::do_wait_touch(int runner_id){
	...
	const float& player_dir = worldModel.get_our_player_dir(runner_id);
...
PlayerTask Shoot::do_stop_ball_and_shoot(int runner_id){
	...
	const point2f& pos = worldModel.get_our_player_pos(runner_id);
	const point2f& ball = worldModel.get_ball_pos();
	const float& dir = worldModel.get_our_player_dir(runner_id);
...
PlayerTask Shoot::do_adjust_dir(int runner_id){
	...
	const point2f& player = worldModel.get_our_player_pos(runner_id);
...
PlayerTask Shoot::plan(int id){
	...
	//射门需要用到的参数
	const point2f& kick_pos = worldModel.get_our_player_pos(id);	// 球员位置
	const point2f& ball = worldModel.get_ball_pos();				// 球的位置

	const point2f& last_ball = worldModel.get_ball_pos(1);			// 获得上一帧球的位置
	const float kicker_dir = worldModel.get_our_player_dir(id);		// 球员角度

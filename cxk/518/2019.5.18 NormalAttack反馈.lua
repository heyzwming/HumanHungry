...
-- 5.18 反馈
-- 1、 elseif   和  else if 是不一样的！ elseif 是在不满足if的条件下执行的判断，如果if满足，则elseif不进行判断。
    -- 而else if 是在不满足if的条件下  再判断一个条件   需要的"end"也不一样
-- 2、 ourTable 是不是没写完？ [3] = "Tier", [4] = "Goalie" ?
-- 3、 获取oppTable的时候应该要将字符串型的value通过tonumber转换成数字再赋值给oppTable
...
oppTable = CGetOppNums()    -- 记着 CGetOppNums()函数返回的是{[0] = "字符串类型的车号", [1] = "字符串类型的车号"}
...
-- 这个程序 当lua每次被调用（1/60s），就会被执行一次，是动态获得的哦。
WhoGetBall = function()
  function GetOurName(num)
...
--reply:是这么声明的吗？
-- 我的意思是 你用到了NoOthers 应该把NoOthers的声明放在前面（我已经移好了）
...
----------------------------------------是否能直接射门-----------------------------------------------
--★★
-- 这个函数为什么要这样写？ 拿球球员的xy和球的xy在一条直线上？的意思吗，所以这怎么判断 到底能否直接射门？
Shoot2Goal = function()
    local x1 = COurRole_y(WhoGetBall)
    local x2 = CGetBallY()
    local y1 = COurRole_x(WhoGetBall)
    local y2 = CGetBallX()
    local k  = math.cot(COurRoleDir(WhoGetBall)) -- 这个cot求k的用法用的牛批！！
    local b  = y1-k*x1
    if y2 == k*x2+b then
        return false
    else
...
["Start"] = {
    switch = function()		
        ...
        elseif WhichGetBall == 0 then
            return "FightForBall"   -- FightForBall 有点好笑哈哈哈哈哈哈哈哈哈
        else     -- 拿到球了
            ...
            while i<4 do
                ...
                if Shoot2Goal==true then --如果能直接射门
                    ...
                    if NoOthers(WhoGetBall,Spy1) then 
                        ...
                        --安排队友捡漏
                        return "Spy1Shoot"
                    elseif NoOthers(WhoGetBall,Spy2) then 
                        WhoGetBall  = task.PassBall(WhoGetBall,Spy2)
                        Spy2        = task.GetBall(Spy2,WhoGetBall)
                        --安排队友捡漏
                        return "Spy2Shoot"
